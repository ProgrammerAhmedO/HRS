...
def rooms(request):
    ...
    firstDayStr = None
    lastDateStr = None
    roomFilter = RoomFilter(request.GET, queryset=rooms)
    rooms = roomFilter.qs
    ...
    if request.method == "POST":
        ...
        if "filter" in request.POST:
            ...
            if (request.POST.get("payment_method") != ""):
                rooms = rooms.filter(
                    payment_method__contains=request.POST.get("payment_method"))

            if (request.POST.get("nob") != ""):
                rooms = rooms.filter(
                    numberOfBeds__contains=request.POST.get("nob"))

            if (request.POST.get("capacity") != ""):
                rooms = rooms.filter(
                    capacity__contains=request.POST.get("capacity"))
...
def add_room(request):
    ...
    if request.method == "POST":
        ...
        room = Room(number=number, capacity=capacity,
                    numberOfBeds=numberOfBeds, roomType=roomType, price=price, payment_method=Payment , 
                    hotel = hotel)
...
def room_profile(request, id):
    ...
    # payment = Room.objects.filter(payment_method=PM)
...
#     Service = RoomServices.objects.get(id=pk)
#     if request.method == "POST":
#         Service.delete()
...
def room_services(request):
    ...
    # x = Booking.objects.get(curBooking.Guest.user)
    ...
    if request.method == "POST":
        if "filter" in request.POST:
            ...
            #         curBooking__lte=request.POST.get("Name"))

            if (request.POST.get("Number") != ""):
                room_services = room_services.filter(
                    room_id__gte=request.POST.get("Number"))
            if (request.POST.get("Type") != ""):
                room_services = room_services.filter(
                    servicesType__contains=request.POST.get("Type"))

            if (request.POST.get("Date") != ""):
                room_services = room_services.filter(
                    createdDate__gte=request.POST.get("Date"))
...
def booking_make(request):
    ...
    if request.method == 'POST':
        ...
        if 'bookGuestButton' in request.POST:
            ...
            curbooking.save()
            if request.POST.get('cname') == '' or request.POST.get('ccnum') == '' or request.POST.get('expmonth') == '' or request.POST.get('expyear') == '' or request.POST.get('cvv') == '':
                messages.error(
                        request, '<<Wrong visa info>>')
                return redirect('rooms')
...
def Reports(request):
    ...
    return render(request, 'common_pages/Reports.html', context)
