...
def Payment(request):
    ...
    return render(request, "common_pages/Payment.html", context )

    
   
   
@login_required(login_url='login')
def Storage_Reports(request):
    role = str(request.user.groups.all()[0])
    path = role + "/"
    if role == "guest":
        return redirect("login")
    ...
    return render(request, "common_pages/Storage_Reports.html", context )
...
def Booking_Reports(request):
    ...
    return render(request, "common_pages/Booking_Reports.html", context )
...
def Event_Reports(request):
    ...
    if role == "guest":
        ...
        attends = EventAttendees.objects.all()

        for attends in attends:
            if attends.event.eventType == 'Live Music':
                Live_Music = Live_Music + int(attends.numberOfDependees)
            elif attends.event.eventType == 'Movie':
                Movie = Movie + int(attends.numberOfDependees)
            elif attends.event.eventType == 'Theater':
                Theater = Theater + int(attends.numberOfDependees)
            elif attends.event.eventType == 'Conference':
                Conference = Conference + int(attends.numberOfDependees)
            elif attends.event.eventType == 'Concert':
                Concert = Concert + int(attends.numberOfDependees)
            elif attends.event.eventType == 'Entertainment':
                Entertainment = Entertainment + int(attends.numberOfDependees)
        total = Movie + Theater + Conference + Concert + Entertainment + Live_Music
    ...
    return render(request, "common_pages/Event_Reports.html", context )


@login_required(login_url='login')
def Services_Reports(request):
    role = str(request.user.groups.all()[0])
    path = role + "/"
    if role == "guest":
        return redirect("login")
        ...
        roomServices = RoomServices.objects.all()
        total_services = roomServices.count()
        ServiceNum = RoomServices.numOfServices
        context = {
    ...
    return render(request, "common_pages/Services_Reports.html", context )




@login_required(login_url='login')
def ReportsSplit(request):
    role = str(request.user.groups.all()[0])
    path = role + "/"
    if role == "guest":
        return redirect("login")
    ...
    return render(request, "admin/ReportsSplit.html", context )
